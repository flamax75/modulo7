...
def guardar_en_base_de_datos(canciones):
    ...
    cursor.execute(
        '''SELECT name FROM sqlite_master WHERE type='table' AND name='canciones' ''')
    ...
    for cancion in canciones:
        try:
            cursor.execute(
                'INSERT INTO canciones (cancion, interprete, año, semanas, pais) VALUES (?, ?, ?, ?, ?)', cancion)
        except sqlite3.IntegrityError:
            # Si hay un error de integridad (probablemente debido a un ID duplicado), ignoramos el registro
            continue
...
def ver_en_base_de_datos():
    ...
    if filas:
        ...
        print(tabulate(filas, headers=[
              "ID", "Canción", "Intérprete", "Año", "Semanas", "País", "Idioma"], tablefmt="grid"))
    else:
...
while True:
    ...
    print("2. Ver datos en base de datos")
    ...
    if opcion == "1":
        canciones = obtener_canciones_mas_semanas_numero_uno(url)
        print("Datos extraídos exitosamente.")
    elif opcion == "2":
        ver_en_base_de_datos()
    elif opcion == "3":
        guardar_en_base_de_datos(canciones)
    elif opcion == "4":
        print("¡Hasta luego!")
        break
    else:
